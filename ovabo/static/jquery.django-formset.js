...
(function($) {
    ...
    $.fn.djangoFormset.Formset = (function() {
        function Formset(base, options) {
            ...
            /* Note: Skip the first entry, it's the CSRF token, so use the last one */
            inputName = this.template.find("input,select,textarea").last().attr('name');
            if (!inputName) {
    ...
    $.fn.djangoFormset.Form = (function() {
        ...
        Form.prototype.getDeleteButton = function() {
            return $("<button type='button' class='btn btn-danger btn-xs' data-action='delete-formset'><i class='glyphicon glyphicon-trash'></i> " + this.formset.opts.deleteButtonText + " </button>");
        };
